...
const bashExecutable =
  ...
  fs.existsSync("C:/Program Files/Git/bin/bash.exe")
    ...
    : "bash";
const bashProbe = spawnSync(bashExecutable, ["--version"], {
  encoding: "utf8",
});
if (bashProbe.status !== 0) {
  throw new Error(
    `Bash is required for operations regressions: ${bashProbe.stderr || "not found"}`,
  );
}
...
describe("operations safeguards", () => {
  it("keeps deployment, database, migration, and restore guards enabled", () => {
    ...
    expect(source).toContain("require_supported_bun");
    ...
    expect(source).toContain('validate_database "$candidate" 11');
    ...
    expect(source).toContain("migrate-v10) migrate_database_to_v11 10");
    expect(source).toContain("migrate-v9) migrate_database_to_v11 9");
    expect(source).toContain("migrate-v8) migrate_database_to_v11 8");
    expect(source).toContain("migrate-v7) migrate_database_to_v11 7");
    expect(source).toContain("migrate-v6) migrate_database_to_v11 6");
    expect(source).toContain("migrate-v5) migrate_database_to_v11 5");
    expect(source).toContain("migrate-v4) migrate_database_to_v11 4");
    expect(source).toContain("migrate-v3) migrate_database_to_v11 3");
    expect(source).toContain("migrate-v2) migrate_database_to_v11 2");
  ...
  it("enforces the exact minimum Bun version", () => {
    const result = runBash(String.raw`
      source "$OPS_PATH"
      rejection_log="$(mktemp)"
      mock_bin="$(mktemp -d)"
      trap 'rm -f -- "$rejection_log"; rm -rf -- "$mock_bin"' EXIT
      write_bun() {
        printf '#!/usr/bin/env bash\nprintf "%%s" "%s"\n' "$1" > "$mock_bin/bun"
        chmod 700 "$mock_bin/bun"
      }
      PATH="$mock_bin:$PATH"
      write_bun 1.3.9
      if (require_supported_bun) 2>"$rejection_log"; then
        exit 91
      fi
      grep -q 'Bun 1.4.0 or newer is required; found 1.3.9' "$rejection_log"
      write_bun 1.4.0
  ...
  it("stashes tracked changes without moving operator files", () => {
  ...
  it("fails closed when an implicit database has no env file", () => {
    const testRoot = fs.mkdtempSync(
  ...
  it("refuses to strand the former default database", () => {
    const testRoot = fs.mkdtempSync(
  ...
  it("creates private files and restricts SQLite sidecars", () => {
    const testRoot = fs.mkdtempSync(
  ...
  it("refuses to restore from the live database itself", () => {
    const testRoot = fs.mkdtempSync(
    ...
    try {
      const result = runBash(
        String.raw`
          ...
          prepare_operation_paths() { :; }
          resolve_runtime_db_file() { :; }
          require_supported_bun() { BUN_EXECUTABLE=bun; }
  ...
  it("leaves the service stopped after a failed v2-to-v11 migration", () => {
    const testRoot = fs.mkdtempSync(
    ...
    try {
      const result = runBash(
        String.raw`
          ...
          bun() { return 1; }
          if migrate_database_to_v11 2; then
            exit 91
          fi
  ...
  for (const databaseBasename of [".hidden.db", "rejected-restore.db"]) {
    it(`recovers a prior ${databaseBasename} after restore validation fails`, () => {
      const testRoot = fs.mkdtempSync(
