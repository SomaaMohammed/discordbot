...
import { initializeV3Schema } from "../src/storage/schema.js";
import {
...
describe("validated SQLite backup", () => {
  it("copies and validates schema v4 including operational data", async () => {
    const root = makeRoot();
    ...
    expect(validateDatabaseFile(source, { expect: 4 }).schema).toBe(
      "current-v4",
    );
    ...
    try {
      ...
      expect(
        copied
          .prepare("SELECT COUNT(*) AS count FROM ticket_configurations")
          .get(),
      ).toEqual({ count: 1 });
      expect(
