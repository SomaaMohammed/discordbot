...
- A missing or empty database is initialized as schema v6. Startup does not
  upgrade an older schema automatically.
- Never share .env, a database, a sidecar, export, or backup. Schema-v6 files
...
SCHEMA UPGRADE
Version 5.4.0 will not start against schema v5, v4, v3, or v2. Close every old bot,
bundled node.exe, and SQLite editor; keep the original database untouched; and
copy it into this folder as superior.db. For the normal schema-v5 upgrade, run
...
leaves the copied source at v5. It preserves every existing row and adds empty
restricted-ping tables and indexes. Keep the validated v5 backup until the
migrated copy, existing workflows, and new restricted-ping command surface have
been reviewed.

Exact schema v4, v3, and the exact supported schema-v2 layout can migrate
directly to v6. Use the same commands with --expect 4, --expect 3, or --expect 2
for the source check and backup, a matching backup name, and --expect 6 for the
final check. The historical v4-to-v5 stage converts the prior ticket model;
v3/v2 retain their documented compatibility behavior. Every legacy path starts
with no restricted-ping mappings. Schema v1 must first be upgraded to v2 with
...
Confirm /restrictedping list is empty, then add only reviewed safe mappings and
test /pingrole plus both cooldowns. Migration never invents or enables external
bindings. Restart command synchronization must register /pingrole and
/restrictedping; global propagation can take time.

UPDATING
1. Stop the bot and create a validated schema-v6 backup.
...
For a release rollback from schema v6 to an older application, stop every
writer and restore that release's original generation-matched pre-migration
backup through a controlled file replacement. The v6 restore tool intentionally
accepts only v6 backups. Restricted-ping state cannot be carried back to v5.
