...
- A missing or empty database is initialized as schema v8. Startup does not
  upgrade an older schema automatically.
- Never share .env, a database, a sidecar, export, or backup. Schema-v8 files
  can contain persistent panel bindings, ticket answers, public suggestions
  and voter IDs, private staff-application answers, and restricted-ping
  member/audit records.
...
SCHEMA UPGRADE
Version 6.0.0 will not start against schema v7, v6, v5, v4, v3, or v2. Close
...
schema-v7 upgrade from Superior 5.5.0, run these commands in PowerShell:
...
leaves the copied source at v7. It preserves every operational row and stored
panel/message/channel/workflow/component ID while converting guild settings
to the active-by-default model. Keep the validated v7 backup until the
migrated copy and existing workflows have been reviewed.

Exact schema v6, v5, v4, v3, and the exact supported schema-v2 layout can
migrate directly to v8. Use the same commands with --expect 6, --expect 5,
--expect 4, --expect 3, or --expect 2 for the source check and backup, a
matching backup name, and --expect 8 for the final check. Historical stages retain their
...
UPDATING
1. Stop the bot and create a validated schema-v8 backup.
...
The v8 restore tool intentionally accepts only v8 backups. After this
installation migrates, never start, deploy, test, or recommend an older
executable for it. For incident recovery, stop every writer, preserve the v8
database and its backups, and use a known-good current-version build, current
