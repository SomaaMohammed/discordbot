...
The schema records a principal type so future versions can add individual-user grants, but schema v8 creates role grants only. `/access` mutations remain owner-or-Administrator-only and reject `@everyone`, managed, missing, duplicate, and cross-guild roles. Workflow-specific support/reviewer roles and delegated capabilities share the same fresh-verification boundary without merging configuration and private-content authority.
...
Schema v8 has exactly 25 application tables and no application views or triggers:
...
| Internal delivery  | One internal delivery-deduplication table                                                                              | Bounded guild/message reservations and terminal outcomes for at-most-once external delivery                       |
...
All interaction and administrative queries have explicit bounds, count/exists variants, or pagination. Ticket, suggestion, and application audit logs retain the latest 100 events per parent; appending a new event trims the oldest in the same transaction instead of blocking a lifecycle action. Restricted-ping administrative audit survives role/mapping deletion, while successful-ping events retain the latest 10,000 entries per guild and remain paginated. Internal delivery-deduplication records retain at most 30 days and 10,000 rows per guild. Format-6 imports additionally cap every portable collection, including restricted roles, mappings, cooldowns, and events; the established limits for grants, departments/fields, panels, tickets, suggestions/votes, applications/forms/responses/events, and metrics remain enforced. These are import safety ceilings, not promises that a deployment should routinely approach them.

## Initialization and migration

A missing or empty database is initialized transactionally at v8. Startup accepts exact v8 only and refuses v1, v2, v3, v4, v5, v6, v7, malformed, partial, and unknown layouts. Schema upgrades are explicit offline operator actions.

The v7-to-v8 migration rebuilds only the guild-settings representation inside one immediate transaction. It preserves every operational table row and all posted-panel, message, channel, workflow, and component identifiers byte-for-byte. Joined guilds become active under settings v3; departed guilds remain inactive. Existing timezone, log channel, invocation, limits, and greetings are retained, with one neutral `Welcome` profile supplied only when the prior list is empty. Any injected or real failure rolls the entire database back to exact v7 state.

The v6-to-v7 migration is additive. In one immediate transaction it validates the complete v6 source, creates one empty bounded internal delivery-deduplication table and two indexes, records schema version 7, and validates the resulting v7 layout. It preserves every v6 row. A failure rolls back the complete transaction and leaves the source at v6.
...
Exact v6, v5, v4, v3, and supported v2 sources migrate through their frozen historical stages and then the v7-to-v8 conversion within the same outer transaction. V3 preserves guild/settings/metrics and begins with no Phase 1 operational rows; v2 keeps safely convertible active data under its existing compatibility rules and refuses unresolved moderation-recovery metadata. Every supported legacy path reaches v8 with empty newly introduced operational tables and active joined-guild defaults. V1 must first be upgraded to the supported v2 layout with the final 4.0.0 release. See [Operations](operations.md) for commands and rollback.

## Guild export and import

Guild export format 6 includes the complete portable tenant product model, subject to collection bounds. Internal delivery-deduplication rows are deliberately excluded and preserved across imports. Parsing validates guild ownership, limits, unique identities, enum/text constraints, and cross-record references before mutation.

- Format 6 is a complete transactional replacement of settings, metrics, and all current operational collections, including restricted-ping configuration, mappings, cooldowns, and audit history.
- Legacy format 5 replaces its schema-v7 model and converts frozen settings v2 into the current settings v3 defaults.
...
Every import leaves the core guild bot active. Inserted grants are inactive; external departments/forms, suggestion configuration, restricted-ping roles, and imported Discord bindings remain disabled or unverified until their current resource/permission checks succeed. Historical workflow and restricted-ping audit records remain for integrity/recovery, but imports never activate external roles or channels automatically.
...
```bash
npm run db:check -- --db /path/to/database.db --expect 8
npm run db:backup -- --db /path/to/database.db --out /new/path/backup.db --expect 8
npm run migrate -- --db /path/to/database.db --dry-run
```

Use `--expect 8` for an active database and `--expect 7`, `6`, `5`, `4`, `3`, or `2` only to validate the corresponding migration source. The backup destination must not already exist. Omit `--dry-run` only during a stopped, validated, backed-up maintenance window.
