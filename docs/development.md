...
The maintained application is a strict TypeScript project in `tsbot/`. Superior 5.5.0 requires Node.js 22.12.0 or newer.
...
The schema records a principal type so future versions can add individual-user grants, but 5.5.0 creates role grants only. `/access` mutations remain owner-or-Administrator-only and reject `@everyone`, managed, missing, duplicate, and cross-guild roles. Workflow-specific support/reviewer roles and delegated capabilities share the same fresh-verification boundary without merging configuration and private-content authority.
...
Schema v7 has exactly 25 application tables and no application views or triggers:
...
Exact v5, v4, v3, and supported v2 sources migrate through their frozen historical stages and then the additive v6-to-v7 step within the same outer transaction. V3 preserves guild/settings/metrics and begins with no Phase 1 operational rows; v2 keeps safely convertible active data under its existing compatibility rules and refuses unresolved moderation-recovery metadata. Every legacy path reaches v7 with empty newly introduced tables. V1 must first be upgraded to the supported v2 layout with the final 4.0.0 release. See [Operations](operations.md) for commands and rollback.

## Guild export and import

Guild export format 5 includes the complete portable tenant product model, subject to collection bounds. Internal delivery-deduplication rows are deliberately excluded and preserved across imports. Parsing validates guild ownership, limits, unique identities, enum/text constraints, and cross-record references before mutation.
...
```bash
npm run db:check -- --db /path/to/database.db --expect 7
npm run db:backup -- --db /path/to/database.db --out /new/path/backup.db --expect 7
npm run migrate -- --db /path/to/database.db --dry-run
```

Use `--expect 7` for an active database and `--expect 6`, `5`, `4`, `3`, or `2` only to validate the corresponding migration source. The backup destination must not already exist. Omit `--dry-run` only during a stopped, validated, backed-up maintenance window.
